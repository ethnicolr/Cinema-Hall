...
import { CinemaShowWithHallType } from '../shared/types'

function BuyTickets() {
    const { data: cinemaShow } = useQuery(['cinemaShow'], () =>
        fetchCinemaShowData(id)
    )
    ...
    const fetchCinemaShowData = (
        id?: string
    ): Promise<CinemaShowWithHallType> => {
        return client(`cinemaShows/${id}`)
    }
    ...
    const buyTickets = async () => {
        ...
        const response = await client(`cinemaShows/${id}/tickets`, {
            data: tickets,
        })
        if (response.success) {
            navigate('/cabinet')
    ...
    if (!cinemaShow) return null
    const { hall, price, tickets } = cinemaShow
    return (
        <main className='mt-20 overflow-y-auto'>
            <section
                className='moviePreview flex flex-shrink-0 flex flex-col justify-end w-full bg-cover bg-center h-48'
